function fitness=copiaclassify_ds1_ds2(featureSet, featureString, featSel, C, gamma,w1,w2,w3)
...
fileNamed = ['./resultados/results_',s,'_SVM','_DS1_DS2_FS_' num2str(numfs) 'results.tex'];
...
[p1d, p1t, p2d, p2t] = loadDataAAMI(featureSet, 0);
...
cd('svm_mac')
    ...
    sC1 = find(target.train(:,1)==1);
    peso_c1 = 400/size(sC1,1);
    
    sC2 = find(target.train(:,2)==1);
    peso_c2 = 400/size(sC2,1);
    
    sC3 = find(target.train(:,3)==1);
    peso_c3 = 400/size(sC3,1);
    
    %sC4 = find(target.train(:,4)==1);
    %peso_c4 = 400/size(sC4,1);
    %peso_c4 = peso_c1;
    
    
    %modsel_weight = sprintf( '-w1 %f -w2 %f -w3 %f -w4 %f', peso_c1, peso_c2, peso_c3, peso_c4);
    
    modsel_weight = '-w1 6 -w2 100 -w3 15';
    
    if C == -1
        [newData, newTarget] = undersampling_all_class(10, fs1.train,target.train);
        [best_c,best_g,best_cv,hC] = parameter_optimization(newData,newTarget,modsel_weight);
    else
        if C == 0
            best_c=0.5;
        else
